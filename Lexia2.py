import sys
sys.path.append("C:\\Users\\Deepak\\Downloads\\glexia_ivt_validation\\glexia_ivt_validation\\src")
...
def find_data_files(path="*.csv"):
    ...
    # Filter out intermediate csv files generated from any previous runs.
    inputfiles = []
    for datafile in datafiles:
        if "idtdata" not in datafile:
            inputfiles.append(datafile)
...
def prepare_scatter_graph_filename(file):
    imgfile = ""
    imgfile = "{}_scatter.png".format(extract_file_wo_extn(file))
    return imgfile

# prepare file name to store ivt intermediate results - csv file    
def prepare_intermediate_data_filename(file, tag="left"):
    interfile = ""
    interfile = extract_file_wo_extn(file) + "idtdata" + tag + ".csv"
    return interfile   
...
def generate_graphs(data, data_df, l_fix, r_fix, fs_left_df, fs_right_df, file):
    ...
    # Calculate the SubTest value of the first level of reading.     
    subtest_list = data_df["SubTest"].unique()
    if (len(subtest_list) > 0):
        zoom_subtest = subtest_list[0]
    
    data_zoomedin = data_df.query("SubTest == {} ".format(zoom_subtest) ) # Subset the data for first 1 second
...
def store_markers(markers, source_file, marker_columns, marker_csv_file):
    ...
    try:
        if (path.exists(marker_csv_file)):
            ...
            new_markers_df[marker_columns].to_csv(marker_csv_file, sep=marker_field_sep, header=marker_headers, index=False)
...
def main(args):
    ...
    mypath = "C:\\Users\\Deepak\\Downloads\\glexia_ivt_validation\\glexia_ivt_validation\\data\\laasya_samples.csv"
    #mypath = "C:\\Users\\Abhishek\\Downloads\\glexia_ivt_validation\\glexia_ivt_validation\\data\\laasya_samples.csv"
    
    sample_freq            = 40 #25 Hz (time between samples in milliseconds)
    IDT_DISPERSSION_THRESHOLD = 7
    ...
    for file in datafiles:
        ...
        if (data.shape[0] < 1):
            print("WARN: No records to process: {0}".format (file) )
            continue
        ...
        #
        idtobj      = idt.PupilIDT(sample_freq=40, threshold_disperssion=IDT_DISPERSSION_THRESHOLD)
       # data_df     = idtobj.calc_horizontal_velocity(data)
        # Merge adjacent fixations and saccades for left eye
        fs_left_df  = idtobj.process_fixation_saccades(data_df, tag="left")
        # Merge adjacent fixations and saccades for right eye
        fs_right_df = idtobj.process_fixation_saccades(data_df, tag="right")
        # Mark saccades as forward or reverse for left eye
        fs_left_df  = idtobj.calc_saccade_direction(data_df, fs_left_df, tag="left")
        # Mark saccades as forward or reverse for right eye
        fs_right_df = idtobj.calc_saccade_direction(data_df, fs_right_df, tag="right")
        
        # Storing intermediate data processed by IVT algorithm. 
        # may be used for fine tuning and debugging purposes. 
        # One file each for left and right eye. Named after <original_file>ivtdata<left/right>.csv
        #
        left_idt_data_file = prepare_intermediate_data_filename(file, tag="left")
        ...
        right_idt_data_file = prepare_intermediate_data_filename(file, tag="right")
        ...
        print("INFO: Storing IDT (intermediate data, calculations ) file: " + calc_data_file)
        ...
        marker_name = "I-DT Disperssion Threshold"
        markers[marker_name] = IDT_DISPERSSION_THRESHOLD
        marker_columns.append(marker_name)
        print("INFO: Assigned idt threshold")
        ...
        marker_name = "student"
        markers[marker_name] = os.path.basename(file)
        marker_columns.append(marker_name)
        print("INFO: Assigned student name")
       ...
       # store_markers(markers=markers, source_file=file, marker_columns=marker_columns, marker_csv_file = markers_file)
        ...
        scatter_plotter.plot_fixation_scatter_chart(data_df, file, prepare_scatter_graph_filename(file))
